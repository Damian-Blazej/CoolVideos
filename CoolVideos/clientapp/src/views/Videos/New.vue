        ...
        <b-container>
            <b-form>
                    ...
                    <b-col md="6">
                        ...
                        <b-form-group label="Opis"
                            ...
                            <b-form-textarea id="description"
                                             ...
                                             rows="3"
                                             max-rows="10"
                                             :state="!$v.video.description.$invalid"></b-form-textarea>
                        ...
                        <img :src="newImageSrc != null ? newImageSrc : `${$serverUrl}/Resources/Images/${video.image}`" style="width: 300px;" />
                        <b-form-file class="mt-2 w-75"
                                     id="imageFile"
                                     accept="image/jpeg"
                                     v-model="newImageFile"
                                     placeholder="Wybierz nowe zdjęcie albo upuść je tutaj..."
                                     drop-placeholder="Upuść zdjęcie tutaj..."
                                     browse-text="Przeglądaj"></b-form-file>
                    </b-col>
                    <b-col md="6" class="d-flex flex-column align-items-end">
                        <video :src="newVideoSrc != null ? newVideoSrc : `${$serverUrl}/Resources/Videos/${video.uri}`" controls type="video/mp4" width="100%" height="auto"></video>
                        <b-form-file class="mt-2"
                                     id="videoFile"
                                     accept="video/mp4"
                ...
                <div class="text-center">
                    ...
                    <b-button id="submitButton" @click="saveNewVideo" class="w-50 mt-5" squared variant="primary">Opublikuj</b-button>
                </div>
            </b-form>
        </b-container>
    </div>
    ...
    export default {
        ...
        methods: {
            saveNewVideo: function () {
                ...
                this.$http.post(this.$serverUrl + `/api/video`, this.video).then(response => {
            ...
            uploadFile: function(file, fileType, filename) {
                ...
                this.$http.post(this.$serverUrl + `/api/file/${fileType}`, formData)
                    .then(res => {
                        return res;
                    })
                    .catch(err => {
