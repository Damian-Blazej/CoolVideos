        ...
        <div class="bg-light text-primary py-5">
            <b-container>
                <div class="text-center">
                    ...
                    <form v-on:submit.prevent="searchVideos(searchQuery, selectedCategory, 1, selectedPageSize)" v-on:keyup.enter="submit">
                ...
                <div class="mt-3 text-center">
                    <b-button @click="showSearchOptions = !showSearchOptions" variant="outline-primary" squared class="font-weight-bold">
                    ...
                    <vue-slide-up-down :active="showSearchOptions" :duration="1000">
                        <div class="mt-3">
                            <b-nav pills align="center" variant="secondary">
                                <b-nav-item :active="category.id === selectedCategory" @click="changeCategory(category.id)" v-for="category in categories" :key="category.id">{{ category.name }}</b-nav-item>
                            </b-nav>
                            <div class="d-flex justify-content-center align-items-center mt-3">
                                <h5 class="font-weight-bold mr-3">Liczba pozycji na stronę</h5>
                                <b-form-select id="selectPageSize" v-model="selectedPageSize" :options="pageSizeOptions" class="text-center" style="width: 75px;"></b-form-select>
                            </div>
                        </div>
                    </vue-slide-up-down>
                </div>
...
<script>
    ...
    export default {
        ...
        created() {
            this.$http.get(this.$serverUrl + "/api/category")
                .then(res => {
            ...
            this.searchVideos(this.searchQuery, this.selectedCategory, 1, this.currentPageSize);
        },
        data() {
            return {
                videos: [],
        ...
        methods: {
            ...
            searchVideos(query, category, pageNum, pageSize) {
                ...
                let url = this.$serverUrl + `/api/video/search?categoryID=${category}&pageSize=${pageSize}&pageNumber=${pageNum}`;
                if (searchQuery)
            ...
            prevPage() {
                if (this.currentPage > 1)
                    this.searchVideos(this.currentQuery, this.currentCategory, --this.currentPage, this.currentPageSize);
            },
            nextPage() {
                if (this.currentPage < Math.ceil(this.currentTotalVideoCount / this.currentNumOfPages))
                    this.searchVideos(this.currentQuery, this.currentCategory, ++this.currentPage, this.currentPageSize);
            },
        },
        computed: {
