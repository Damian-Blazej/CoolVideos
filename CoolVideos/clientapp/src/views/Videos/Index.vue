<template>
    <b-container>
        <div class="mt-5 d-flex flex-column align-items-center">
            ...
            <video :src="`${$serverUrl}/Resources/Videos/${video.uri}`" controls width="100%" height="auto"></video>
...
<script>
    ...
    export default {
        ...
        created() {
            fetchData(this.$serverUrl + "/api/video/" + this.$route.params.index)
                .then(data => {
                    if (data.status === 404) {
        ...
        methods: {
            toggleLike: function() {
                if (this.didUserLikeVideo)
                    this.deleteLike();
                else
                    this.giveLike();
            },
            giveLike: function() {
                this.$http.post(this.$serverUrl + '/api/like', { userID: localStorage.getItem("userId"), videoID: this.video.id })
                    .then(res => res.body)
                    .then(res => {
            ...
            deleteLike: function() {
                this.$http.delete(this.$serverUrl + '/api/like', { body: { userID: localStorage.getItem("userId"), videoID: this.video.id }})
                    .then(res => res.body)
                    .then(res => {
            ...
            checkIfUserLikedVideo: function() {
                fetchData(this.$serverUrl + `/api/like/video/${this.video.id}/user/${localStorage.getItem("userId")}`)
