<template>
    <div class="login-body d-flex justify-content-center align-items-center">
        <div class="bg-light px-5 py-5 text-primary shadow d-flex justify-content-center align-items-center flex-column">
            ...
            <b-button id="loginButton" v-on:click="login" variant="outline-warning" block squared class="mt-3 mb-2">Zaloguj</b-button>
            <div>
                <label id="errorLogin" v-if="loginStatus === false"> Błąd logowania</label>
            </div>
            <router-link to="/register" class="my-2">Nie masz jeszcze u nas konta? Zarejestruj się</router-link>
        </div>
    </div>
</template>
...
<script>
    export default {
        ...
        methods: {
            login() {
                this.$http.post(this.$serverUrl + '/api/token', this.credential)
                    .then((result) => {
